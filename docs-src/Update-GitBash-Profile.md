...
# カスタムインストール先を指定してインストール
.\Update-GitBash-Profile.ps1 -Install -InstallDir "D:\MyTools\devbin-win\bin"
...
- `-InstallDir <path>`: devbin-win のインストール先ディレクトリ (既定: `C:\ProgramData\<ユーザー名>\devbin-win\bin`)
...
- **名前**: Git Bash
- **実行ファイル**: `C:\ProgramData\<ユーザー名>\devbin-win\bin\git\bin\bash.exe -i -l`
- **開始ディレクトリ**: `%USERPROFILE%`
- **アイコン**: `C:\ProgramData\<ユーザー名>\devbin-win\bin\git\mingw64\share\git\git-for-windows.ico`
