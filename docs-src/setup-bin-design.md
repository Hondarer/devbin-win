...
旧実装 (Setup-Bin.legacy.ps1) は参照用に保持されています。

**主な違い**:

- 旧実装: パッケージ固有処理が関数内の elseif 分岐として実装
- 新実装: パッケージ情報を定義ファイルで管理、処理を戦略パターンで実装

### ファイルサイズ比較

| ファイル | 行数 | 説明 |
|---------|------|------|
| Setup-Bin.legacy.ps1 | 1735 | 旧実装 (単一ファイル) |
| Setup-Bin.ps1 | 276 | 新実装 (メインスクリプト) |
| Setup-Common.psm1 | 588 | 共通関数モジュール |
| Setup-Strategies.psm1 | 540 | 抽出戦略モジュール |
| packages.psd1 | 194 | パッケージ定義 |
| **合計** | **1598** | **新実装全体** |
