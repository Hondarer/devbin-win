...
| DownloadFileName | URL からファイル名を決定できない場合に packages フォルダへ保存するファイル名 | string | ❌ |
...
`DownloadUrl` の末尾から保存名を決定できない場合は `DownloadFileName` を指定します。たとえば VS Code の公式固定版 URL は末尾が `stable` になるため、`DownloadFileName = "VSCode-win32-x64-1.117.0.zip"` のように指定します。
