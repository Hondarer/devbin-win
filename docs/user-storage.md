...
| `agy` | `JETSKI_APP_DATA_DIR=data\agy`。既定は `~/.gemini/antigravity-cli`。この変数は実行ファイル内で確認したもので、公式ドキュメントには記載がなく動作実証もしていない。資格情報ストアと `~/.gemini/config` は対象外 |
