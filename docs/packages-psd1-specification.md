...
`python -m pip install` を実行して Python パッケージをインストールします。アーカイブファイルは不要で、`DownloadUrl` も省略します。
...
`ArchivePattern = "^$"` はアーカイブ検索をスキップするための規約です。`DownloadUrl` は省略します (pip が依存解決とダウンロードを担うため)。オフライン用 wheel は `Get-Packages.ps1` が `packages/pip-packages/` に保存します。
...
`Get-Packages.ps1` は npm パッケージ本体と `NpmDependencies` を `packages/npm-packages/*.tgz` として保存します。PowerShell の `ni` は `New-Item` alias と衝突するため、必要な場合はユーザー側で `Remove-Item Alias:ni -Force` を実行してください。
